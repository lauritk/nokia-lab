...
% Modified and fixed to remap channels to ascending order

%fname       = fullfile(ops.fpath, sprintf('%s.dat', ops.fbinary)); 
fname       = ops.fbinary; 
fidout      = fopen(fname, 'w');
%

% Loads remapping configuration from channel map
channels = load(ops.chanMap);
remap = channels.remap_config;

clear fs
for j = 1:ops.Nchan
    % load only selected channels, remaps also to ascending order
	fs{j} = dir(fullfile(ops.root, sprintf(ops.ephyNamingStyle, remap(j) )));
end
